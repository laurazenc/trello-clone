...
describe('Lists > Delete List', () => {
  const server = new TestServer();

  it('should return error if user is not authorized', async () => {
    const result = await server.deleteList(list._id);
  ...
  it('should return error if list does not exist', async () => {
    ...
    const result = await server.deleteList(id.toString());
    expect(result.data.deleteList.errors[0].message).toBe(listMessages.listDoesNotExist);
  });

  it('should return true if the list has been removed', async () => {
    await server.login(user.email, password);
    const result = await server.deleteList(list._id);
    expect(result.data.deleteList.result).toBeTruthy();
  });

  it('should delete list from board', async () => {
    await server.login(user.email, password);
